   ...
   function __construct() {
    ...
    public function index() {
        ...
        if ($this->session->userdata('admin_login') == 1)
            redirect(base_url() . 'index.php?admin/minhas_disciplinas', 'refresh');
    ...
    function valida_login() {
        ...
        if ($query->num_rows() > 0) {
            ...
            $row = $query->row();
            $this->session->set_userdata('professor_id_sessao', $row->professor_id);
            $this->session->set_userdata('admin_login', '1');
            $this->session->set_userdata('login2', $row->login);
           ...
           // $this->session->set_userdata('login_type', 'admin');
            ...
            $data['ip'] = $ip;
            $data['acao'] = 'LOGIN COM SUCESSO! - Portal Professor ';
            $data['login'] =  $login;
         ...
         redirect(base_url() . 'index.php?admin/minhas_disciplinas', 'refresh');
            ...
            $data['dt_login'] = $data_login;
            $data['ip'] = $ip;
            $data['acao'] = 'TENTATIVA DE LOGIN SEM SUCESSO! - Portal Professor ';
            $data['login'] =  $login;
            $this->db->insert('log_login', $data);
            $log_login_id = mysql_insert_id();
